...
* **[NoteBoard 留言板模式](README_NOTEBOARD.md)**：
    ...
    * **離線地圖**：支援 MBTiles 格式離線地圖，無網路環境下可使用地圖定位功能。
    * **ePaper 電子紙顯示**：可連接 Waveshare ePaper 模組，將留言板內容自動顯示於電子紙螢幕。
...
- [x] 簡易安裝腳本
- [ ] Image 版本安裝
- [ ] 提供 Web 介面設定 WiFi SSID、LoRa 參數等。
- [ ] 系統更新
- [ ] 英文版文件
- [ ] 類BBS系統
- [ ] 系統公告
...
Noteboard模組
- [x] **多頻道與權限管理**：支援多個獨立頻道，每個頻道可設定進入密碼、管理者密碼與發文通關碼
- [x] **離線地圖**：整合 MBTiles 格式離線地圖，支援多檔載入與圖層套疊
- [x] **ePaper 電子紙顯示**：整合 Waveshare ePaper 模組，自動截圖顯示留言板內容
- [ ] **多國語言支援**：操作介面支援多國語言
- [ ] **節點管理**：可查看已知的節點狀態
- [x] **自動重新發送**：當訊息接收節點數量，未如預期，自動發重新發送
- [ ] **訊息發送者認證**：透過數位簽章，確認訊息發送者身份
