...
- **地圖檢視（MapView）**：留言可附加地理座標，系統自動解析並在離線地圖上以標記點顯示位置。
- **離線地圖支援**：支援 MBTiles 格式的離線地圖，可在無網路環境下使用地圖功能
- **表格檢視（TableView）**：支援類似試算表的資料呈現方式，可建立多個工作表（Sheet），每個儲存格可獨立編輯內容與顏色。管理者可新增、重新命名或刪除工作表。
- **LoRa資料傳輸**：使用 Meshtastic 網路傳輸資料，可使用現有 Mesh 網路的節點與頻道，也可獨立建立新的網路
- **節流傳輸**：針對 LoRa 的有限傳輸頻寬，設計的流量控制傳輸機制：所有待傳輸的訊息列隊在資料庫中，依照所設定的發送頻率依序發送。
- **自動補送功能**：當訊息發送後，若各節點回報的接收數量不足，系統將自動重新發送，確保訊息傳達至所有節點
...
管理者為具有該頻道管理權限的角色，需透過管理者密碼認證後取得。
